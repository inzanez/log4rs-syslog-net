...
#[doc(hidden)]
pub fn level_to_severity(lvl: Level) -> u8 {
    match lvl {
        Level::Error => Severity::ERROR as u8,
        Level::Warn => Severity::WARNING as u8,
        Level::Info => Severity::INFO as u8,
        Level::Debug => Severity::DEBUG as u8,
        Level::Trace => Severity::DEBUG as u8,
    }
}
    ...
    let res = match f.to_lowercase().as_str() {
        ...
        _ => {
            return Err(io::Error::new(
                io::ErrorKind::Other,
                format!("Unsupported facility {}", f).as_str(),
            ))
        }
    };
    Ok(res)
